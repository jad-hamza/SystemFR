...
Lemma reducible_value_let:
  forall ρ v t A B,
    ...
    is_erased_type B ->
    reducible_values ρ v A ->
    reducible ρ (open 0 t v) (open 0 B v) ->
    reducible ρ (app (notype_lambda t) v) (open 0 B v).
Proof.
  steps.
...
Lemma reducible_let:
  forall ρ t1 t2 A B,
    ...
    (forall v,
        cbv_value v ->
        star scbv_step t1 v ->
        reducible ρ (open 0 t2 v) (open 0 B v)) ->
    reducible ρ (app (notype_lambda t2) t1) (open 0 B t1).
Proof.
  unfold reduces_to, closed_term; repeat step || list_utils; eauto with fv.
  createHypothesis;
    repeat step || values_info.
...
Lemma reducible_let2:
  forall ρ t1 t2 A B,
    ...
    (forall v,
        cbv_value v ->
        star scbv_step t1 v ->
        reducible ρ (open 0 t2 v) B) ->
    reducible ρ (app (notype_lambda t2) t1) B.
Proof.
  intros.
...
Proof.
  ...
  eapply reducible_let;
   repeat step || top_level_unfold reducible || top_level_unfold reduces_to ||
          values_info || t_deterministic_star || t_termlist || t_instantiate_sat4;
