...
Lemma reducible_type_abs:
  forall ρ t T X,
    ...
    (forall RC,
      ...
      reducible ((X,RC) :: ρ) t (topen 0 T (fvar X type_var))) ->
    reducible ρ t (T_abs T).
Proof.
  intros.
...
Lemma open_reducible_type_abs:
  forall Θ Γ t T (X : nat),
    ...
    (X ∈ Θ -> False) ->
    is_erased_term t ->
    open_reducible (X :: Θ) Γ t (topen 0 T (fvar X type_var)) ->
    [ Θ; Γ ⊨ t : T_abs T ].
Proof.
...
Proof.
  ...
  | H: forall RC, reducibility_candidate RC -> _ |- _ =>
      unshelve epose proof (H (fun v => reducible_values ρ v V) _); steps;
