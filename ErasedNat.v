...
Lemma reducible_value_zero:
  forall ρ, reducible_values ρ zero T_nat.
Proof.
  repeat step || simp_red.
Qed.

Lemma reducible_zero:
  forall ρ, reducible ρ zero T_nat.
Proof.
...
Lemma reducible_values_succ:
  forall ρ v,
    reducible_values ρ v T_nat ->
    reducible_values ρ (succ v) T_nat.
Proof.
...
Lemma reducible_match:
  forall ρ tn t0 ts T,
    ...
    is_erased_term ts ->
    valid_interpretation ρ ->
    reducible ρ tn T_nat ->
    (equivalent_terms tn zero -> reducible ρ t0 T) ->
     (forall n,
        equivalent_terms tn (succ n) ->
        reducible_values ρ n T_nat ->
        reducible
          ρ
          (open 0 ts n)
          T) ->
    reducible ρ (tmatch tn t0 ts) T.
Proof.
  steps.
