...
Equations (noind) reducible (theta: interpretation) (v: tree) (T: tree): Prop
    by wf (get_measure T) lt_measure :=
  reducible theta v (fvar X type_var) :=
    match lookup Nat.eq_dec theta X with
    | None => False
    | Some P => P v
    end;

  reducible theta v T_unit := v = uu;

  reducible theta v T_bool := v = ttrue \/ v = tfalse;

  reducible theta v T_nat := is_nat_value v;

  reducible theta v (T_abs T) :=
    closed_value v /\
    exists X,
      ~(X ∈ support theta) /\
      ~(X ∈ pfv T type_var) /\
      forall RC,
        ...
        reducible ((X,RC) :: theta) v (topen 0 T (fvar X type_var));

  reducible theta v (T_arrow A B) :=
    ...
    forall a (p: is_erased_term a),
      ...
      pfv a term_var = nil ->
      reducible theta a A ->
      reduces_to (fun t => reducible theta t (open 0 B a)) (app v a);

  reducible theta v (T_prod A B) :=
    exists (_: is_erased_type B),
    closed_value v /\
  ...
  reducible theta v (T_sum A B) :=
    closed_value v /\ (
      (exists v', v = tleft v' /\ reducible theta v' A) \/
      (exists v', v = tright v' /\ reducible theta v' B)
    );

  reducible theta v (T_refine T p) :=
    closed_value v /\
    reducible theta v T /\
    is_erased_term p /\
    wf p 1 /\
  ...
  reducible theta v (T_type_refine T1 T2) :=
    exists (_: is_erased_type T2),
    exists (_: closed_value v),
      reducible theta v T1 /\
      exists p, reducible theta p (open 0 T2 v);

  reducible theta v (T_intersection A B) :=
    closed_value v /\
    reducible theta v A /\
    reducible theta v B;

  reducible theta v (T_union A B) :=
    closed_value v /\ (
      reducible theta v A \/
      reducible theta v B
    );

  reducible theta v T_top :=
    closed_value v;

  reducible theta v T_bot := False;

  reducible theta v (T_equiv t1 t2) :=
    v = uu /\
    equivalent_terms t1 t2;

  reducible theta v (T_forall A B) :=
    exists (_: is_erased_type B),
    closed_value v /\
    forall a (p: is_erased_term a),
      wf a 0 ->
      pfv a term_var = nil ->
      reducible theta a A ->
      reducible theta v (open 0 B a);

  reducible theta v (T_exists A B) :=
    exists (_: is_erased_type B),
    closed_value v /\
  ...
  reducible theta v (T_rec n T0 Ts) :=
    closed_value v /\
    is_erased_term n /\ (
      (star scbv_step n zero /\ reducible theta v T0) \/
      (exists n' X (p1: is_nat_value n') (p2: star scbv_step n (succ n')),
         ~(X ∈ pfv T0 type_var) /\
         ~(X ∈ pfv Ts type_var) /\
         ~(X ∈ support theta) /\
         reducible ((X, fun t => reducible theta t (T_rec n' T0 Ts)) :: theta)
                          v
                          (topen 0 Ts (fvar X type_var))
      )
    );

  reducible theta v T := False
.
...
Fail Next Obligation. (* no more obligations for reducible *)

Definition reducible (theta: interpretation) t T : Prop :=
  reduces_to (fun t => reducible theta t T) t.

Definition subtype theta T1 T2 :=
  forall v, reducible theta v T1 -> reducible theta v T2.

Definition open_reducible (tvars: tvar_list) (gamma: context) t T : Prop :=
  forall theta lterms,
    valid_interpretation theta ->
    satisfies (reducible theta) gamma lterms  ->
    support theta = tvars ->
    reducible theta
              (substitute t lterms)
              (substitute T lterms).
...
Definition open_subtype (tvars: tvar_list) (gamma: context) T1 T2 : Prop :=
  forall theta l,
   valid_interpretation theta ->
   satisfies (reducible theta) gamma l ->
   support theta = tvars ->
...
Ltac simp_red_hyp :=
  match goal with
  | H: _ |- _ => rewrite_strat outermost hints reducible in H
  end.

Ltac simp_red_top_level_hyp :=
  match goal with
  | H: _ |- _ => rewrite_strat hints reducible in H
  end.

Ltac simp_red_goal := rewrite_strat outermost hints reducible.

Ltac simp_red := simp_red_hyp || simp_red_goal.

Ltac simp_red_nat :=
  match goal with
  | H: reducible _ _ T_nat |- _ => simp reducible in H
  end.
