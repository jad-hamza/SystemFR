...
Definition non_empty ρ A := exists v, reducible_values ρ v A.

Lemma instantiate_non_empty:
  forall ρ A,
    non_empty ρ A ->
    exists a, reducible_values ρ a A.
Proof.
...
Lemma push_is_candidate:
  forall (ρ : interpretation) (A a : tree) (RC : tree -> Prop),
    ...
    reducibility_candidate (fun v : tree => reducible_values ρ a A -> RC v).
Proof.
...
Lemma push_all_is_candidate:
  forall (ρ : interpretation) (A : tree) (RC : tree -> Prop),
    reducibility_candidate RC ->
    non_empty ρ A ->
    reducibility_candidate (fun v : tree => forall a, reducible_values ρ a A -> RC v).
Proof.
  repeat step || unfold non_empty in * || unfold reducibility_candidate in * || instantiate_any;
    eauto with fv wf.
Qed.
...
Lemma forall_implies_apply:
  forall P pre_ρ ρ X pre_rc rc v,
    forall_implies P pre_ρ ρ ->
    lookup Nat.eq_dec pre_ρ X = Some pre_rc ->
    lookup Nat.eq_dec ρ X = Some rc ->
...
Ltac t_forall_implies_apply :=
  match goal with
  | H1: forall_implies ?P ?pρ ?ρ,
    H2: lookup _ ?pρ ?X = Some ?prc,
...
Ltac t_forall_implies_support :=
  match goal with
  | H: forall_implies ?P ?pρ ?ρ |- _ =>
    poseNew (Mark (pρ,ρ) "forall_implies_suppoft");
    pose proof (forall_implies_support _ _ _ H)
  end.

Lemma forall_implies_equiv:
  forall P1 P2 pρ ρ,
    forall_implies P1 pρ ρ ->
    (forall x, P1 x <-> P2 x) ->
    forall_implies P2 pρ ρ.
Proof.
  induction pρ; destruct ρ; steps; eauto with eapply_any.
Qed.

Ltac t_forall_implies_equiv :=
  match goal with
  | H1: forall_implies ?P1 ?pρ ?ρ |- forall_implies _ ?pρ ?ρ =>
