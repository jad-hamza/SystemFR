...
Lemma reducible_values_rec_step:
  forall ρ t1 t2 T0 Ts v,
    reducible_values ρ v (T_rec t1 T0 Ts) ->
    star scbv_step t1 t2 ->
    reducible_values ρ v (T_rec t2 T0 Ts).
Proof.
...
Lemma reducible_rec_equivalent:
  forall ρ t1 t2 T0 Ts t,
    reducible ρ t (T_rec t1 T0 Ts) ->
    valid_interpretation ρ ->
    equivalent_terms t1 t2 ->
    reducible ρ t (T_rec t2 T0 Ts).
Proof.
  eauto using reducible_values_rec_equivalent, reducible_values_exprs.
Qed.

Lemma same_relation_rec_step:
  forall ρ t1 t2 T0 Ts,
    is_erased_term t1 ->
    star scbv_step t1 t2 ->
    same_relation
      (fun t => reducible_values ρ t (T_rec t1 T0 Ts))
      (fun t => reducible_values ρ t (T_rec t2 T0 Ts)).
Proof.
...
Lemma reducible_values_unfold:
  forall ρ v n T0 Ts,
    ...
    pfv Ts term_var = nil ->
    valid_interpretation ρ ->
    reducible_values ρ v (T_rec (succ n) T0 Ts) ->
    reducible_values ρ v (topen 0 Ts (T_rec n T0 Ts)).
Proof.
  unfold reduces_to;
  ...
  apply same_relation_sym; apply same_relation_rec_step; eauto with erased.
Qed.

Lemma red_one:
  forall ρ, reducible_values ρ (succ zero) T_nat.
Proof.
  repeat step || simp_red.
Qed.
...
Ltac inst_one :=
  match goal with
  | H: forall a, reducible_values ?ρ _ T_nat -> _ |- _ =>
...
Lemma reducible_unfold:
  forall ρ t n T0 Ts,
    ...
    pfv Ts term_var = nil ->
    valid_interpretation ρ ->
    reducible ρ t (T_rec (succ n) T0 Ts) ->
    reducible ρ t (topen 0 Ts (T_rec n T0 Ts)).
Proof.
  unfold reduces_to;
    repeat match goal with
...
Proof.
  ...
  | |- reducible_values ((?M,_) :: _) _ _ =>
    eapply (reducible_rename_one_rc (fun v : tree => reducible_values ρ v (T_rec n T0 Ts)) _ _ _ _ M M);
    repeat step || apply same_relation_rec_step
  end;
...
Lemma reducible_unfold_zero:
  forall ρ t T0 Ts,
    ...
    is_erased_type Ts ->
    valid_interpretation ρ ->
    reducible ρ t (T_rec zero T0 Ts) ->
    reducible ρ t T0.
Proof.
  unfold reduces_to;
    repeat match goal with
...
Lemma reducible_unfold_in:
  forall t1 t2 T n T0 Ts ρ,
    ...
    pfv Ts term_var = nil ->
    (forall v,
        reducible_values ρ v T0 ->
        equivalent_terms t1 v ->
        equivalent_terms n zero ->
        reducible ρ (open 0 t2 v) T) ->
    (forall v,
        reducible_values ρ v (topen 0 Ts (T_rec (notype_tpred n) T0 Ts)) ->
        equivalent_terms t1 v ->
        reducible ρ (open 0 t2 v) T) ->
    reducible ρ (app (notype_lambda t2) t1) T.
Proof.
  intros.
  ...
  apply (
      reducible_rename_one_rc
        _
        (fun t => reducible_values ρ t (T_rec (notype_tpred n) T0 Ts))
        _ _ _ X X
   ) in H26;
    eauto with values;
    eauto using reducibility_is_candidate.
  ...
  - apply same_relation_sym.
    apply same_relation_rec_step; unfold notype_tpred; steps.
    eapply star_trans; eauto with cbvlemmas.
