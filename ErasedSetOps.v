...
Lemma reducible_intersection:
  forall ρ e T1 T2,
    ...
    reducible ρ e T1 ->
    reducible ρ e T2 ->
    reducible ρ e (T_intersection T1 T2).
Proof.
  unfold reduces_to;
...
Lemma reducible_union:
  forall ρ e T1 T2,
    valid_interpretation ρ ->
    (reducible ρ e T1 \/ reducible ρ e T2) ->
    reducible ρ e (T_union T1 T2).
Proof.
  unfold reduces_to;
    repeat step || t_deterministic_star;
...
Proof.
  ...
  - unshelve epose proof (H15 ρ ((z, v) :: lterms) _ _ _);
      repeat step || list_utils || apply SatCons || values_info || t_deterministic_star || t_substitutions ||
