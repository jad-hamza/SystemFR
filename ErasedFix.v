...
Lemma reducible_fix_zero:
  forall ρ T ts v,
    ...
    (forall tx n,
      ...
      reducible_values ρ tx
                       (T_forall (T_refine T_nat (tlt (lvar 0 term_var) n)) T) ->
      equivalent_terms tx (notype_tfix ts) ->
      reducible ρ (open 0 ts tx) (open 0 T n)) ->
    reducible ρ (notype_tfix ts) (open 0 T zero).
Proof.
  intros.
  ...
  forall n ρ T tsv v ts,
    ...
    (forall tx n,
       ...
       reducible ρ
         (open 0 ts tx)
         (open 0 T n)) ->
    reducible ρ (notype_tfix ts) (open 0 T v).
Proof.
  ...
  forall ρ ts tsv T,
    ...
    (forall tx n,
       ...
       reducible ρ
         (open 0 ts tx)
         (open 0 T n)) ->
    reducible ρ (notype_tfix ts) (T_forall T_nat T).
Proof.
  repeat step.
